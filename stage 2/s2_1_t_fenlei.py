...
from auto_encoder import Decoder_HR_gen, Decoder_HR_dis, Encoder_HR, Encoder_LR_VGG, Encoder_Tac
...
import val_s1_3_t_fenlei as val
...
# 触觉使用配套的512维度序列的时候,对应的需求修改编码网络,修改触觉读取的数据read_split_data_tac_densenet,修改MyDataSet数据
...
path5 = 'D:/Desktop/12.11responce/path_encoder_t'
...
def train():
    ...
    for i in range(n_epochs):
        for j, (data_batch, imgs_lr, tacs, target_batch, filepath) in enumerate(train_loader):
            ...
            data_in_t = image_normal(data_in_t)
            ...
            for name, param in encoder.named_parameters():
                if "densenet121" in name:  # densenet121
                    param.requires_grad = False

            encoder_optimizer.zero_grad()

            encoder_t_z, encoder_t_z_logit = encoder(data_in_t) #[8,9]
            celoss = nn.CrossEntropyLoss()
            loss_encoder = celoss(encoder_t_z_logit, data_target.long())
            loss_encoder.backward(retain_graph=True)
            encoder_optimizer.step()
            acc_ = sumprescise(encoder_t_z_logit, data_target)
            acc = acc_[0]/acc_[1]
            print('epoch[{0}/{1}],bathchid[{2}/{3}],loss_encoder:{4},acc for t:{5}'.format
        ...
        if i % 10 == 0:
            torch.save(encoder.state_dict(), path5)
