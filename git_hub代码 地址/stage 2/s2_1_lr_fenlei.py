...
from auto_encoder import Decoder_HR_gen, Decoder_HR_dis, Encoder_HR, Encoder_LR_VGG
...
import val_s1_2_lr_fenlei as val
...
def train():
    ...
    encoder = Encoder_LR_VGG().cuda()  #没问题
    ...
    for i in range(n_epochs):
        for j, (data_batch, imgs_lr, tacs, target_batch, filepath) in enumerate(train_loader):
            ...
            data_in_lr = image_normal(data_in_lr)
            ...
            encoder_lr_z, encoder_lr_z_logit = encoder(data_in_lr) #[8,9]
            celoss = nn.CrossEntropyLoss()
            loss_encoder = celoss(encoder_lr_z_logit, data_target.long())
            loss_encoder.backward(retain_graph=True)
            encoder_optimizer.step()
            acc_ = sumprescise(encoder_lr_z_logit, data_target)
            acc = acc_[0]/acc_[1]
            print('epoch[{0}/{1}],bathchid[{2}/{3}],loss_encoder:{4},acc--lr:{5}'.format
        ...
        log_writer.add_scalar('acc_lr', acc, step_index)
        step_index = step_index + 1

        if i % 10 == 0:
            torch.save(encoder.state_dict(), path4)
